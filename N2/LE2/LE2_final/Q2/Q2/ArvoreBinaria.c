...
void libera_NO_BST(struct NO_BST* no) {
   // condição de parada
   if (no == NULL)
      return;
   // inicio das recorrências
   libera_NO_BST(no->esq);
   libera_NO_BST(no->dir);
...
void libera_ArvBin(ArvBin* raiz) {
   ...
   libera_NO_BST(*raiz); /* libera cada nó */
   // libera a raiz
   free(raiz);
}
...
int insere_ArBin(ArvBin* raiz, int valor) {
   ...
   // criação do novo nó
   struct NO_BST* novo;
   novo = (struct NO_BST*)malloc(sizeof(struct NO_BST));
   if (novo == NULL)
      return 0;
   novo->info = valor;
   novo->dir = NULL;
   ...
   if (*raiz == NULL) /* Arvore vazia */
      *raiz = novo;
      ...
      struct NO_BST* atual = *raiz;
      struct NO_BST* ant = NULL;
      while (atual != NULL) {
         ant = atual;
         if (valor == atual->info) {
...
struct NO_BST* remove_atual(struct NO_BST* atual) {
   struct NO_BST* no1, * no2; /* ponteiros esquerdo e direito de atual */
...
int consulta_ArvBin(ArvBin* raiz, int valor) {
   ...
   if (raiz == NULL)
      return 0;
   // percurso sequêncial
   struct NO_BST* atual = *raiz;   /* ponteiro ao nó raiz */
   while (atual != NULL) {
      if (valor == atual->info) {
...
// função recursiva que percorre a árvore em pré-ordem imprimindo os dados.
...
// função recursiva que percorre a árvore em pós-ordem imprimindo os dados.
